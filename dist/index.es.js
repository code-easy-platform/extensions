...
class a {
  ...
  activate() {
    this._worker = new Worker(this._url, { type: "module" }), this._workerMessageSender = new n(this._worker), this._workerMessageReceiver = new i(this._worker), this._workerMessageReceiver.onMessage = this._onEvent.bind(this), this._workerMessageSender.send({ type: "activate" }).then(() => this.hasInitialized = !0).catch(() => console.warn("Failed to activate a extension"));
  }
  deactivate() {
    var e;
...
class d {
  constructor(e) {
    this._extensionId = crypto.randomUUID(), this._commands = {}, this._workerReceiver = new i(e), this._workerSender = new n(e), this._commands.activate = this.activate.bind(this), this._commands.deactivate = this.deactivate.bind(this), this._workerReceiver.onMessage = this._onEvent.bind(this);
  }
  activate() {
  }
  deactivate() {
